...
class ExportLevel(str, Enum):
    """Níveis de exportação (baseado na referência de hierarquia de acessos)."""
    AGGREGATED = "aggregated"
    ANONYMIZED = "anonymized"
    FULL = "full"


# Roles canônicas (internas)
ROLE_ADMIN = "admin"
ROLE_AUDIT = "audit"
...
class EndpointPolicy:
    ...
    export_min_level: Optional[ExportLevel] = None  # None => sem export
    allow_export_with_municipio_scope: bool = False  # MVP: False


def endpoint_policy(endpoint: str) -> EndpointPolicy:
    """
    Políticas mínimas por endpoint.
    Ajuste conforme o produto evoluir.
    """
    ...
    if ep == "facts":
        return EndpointPolicy(
            ...
            allow_export_with_municipio_scope=False,
        )
...
def require_export_allowed(user: UserContext, policy: EndpointPolicy, *, endpoint: str, requested_format: str) -> None:
    ...
    # role mínima para export (MVP: admin/audit/strategic/tactical)
    ...
    # MVP: bloquear export com MUNICIPIO:* (até existir filtro seguro)
    if user_has_municipio_scope(user) and not policy.allow_export_with_municipio_scope:
        raise ForbiddenError(
            message="Exportação com escopo municipal ainda não está habilitada (MVP)",
            details={"endpoint": endpoint, "format": fmt, "reason": "needs_safe_export_filter"},
        )
