...
function Example() {
  ...
  return (
    <>
      ...
      <Modal isOpen={modal.isOpen} onClose={modal.close} title="Título do Modal">
...
function Example() {
  ...
  return <Button onClick={() => toast.success('Salvo com sucesso!')}>Salvar</Button>
}
...
;<Table>
  <TableHeader>
    <TableRow>
      <TableCell sortable onSort={handleSort}>
        Nome
      </TableCell>
...
```html
<div class="status-card status-card--success">Card de sucesso</div>
```
...
_Documentação gerada automaticamente. Para contribuir, edite `docs/DESIGN_SYSTEM.md`._
